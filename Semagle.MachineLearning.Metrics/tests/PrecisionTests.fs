...
type ``Precision tests``() =
    [<Test>]
    member _.``Precision should be zero.``() =
        let expected = [| |]
        let predicted = [| |]

        Classification.precision expected predicted
    ...
    [<Test>]
    member _.``Precision should be one.``() =
    ...
    [<Test>]
    member _.``Precision should be correct.``() =
        let expected = [| "A"; "B"; "C"; "A"; "B"; "C" |]
        let predicted = [| "C"; "B"; "C"; "C"; "A"; "C" |]

        Classification.precision expected predicted
        |> should equal (Map.ofArray [| ("A", 0.0); ("B", 1.0); ("C", 0.5) |])
