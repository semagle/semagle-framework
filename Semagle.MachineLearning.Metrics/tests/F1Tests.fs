...
type ``F1 tests``() =
    [<Test>]
    member _.``F1 should be zero.``() =
        let expected = [| |]
        let predicted = [| |]

        Classification.f1 expected predicted
    ...
    [<Test>]
    member _.``F1 should be correct.``() =
        let expected = [| "A"; "B"; "C"; "A"; "B"; "C" |]
        let predicted = [| "C"; "B"; "C"; "C"; "A"; "C" |]

        Classification.f1 expected predicted
        |> should equal (Map.ofArray [| ("A", 0.0);
            ("B", 1.0/(1.0+0.5*(0.0+1.0))); ("C", 2.0/(2.0+0.5*(2.0+0.0))) |])
